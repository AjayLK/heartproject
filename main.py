...
from heart_model.utils import HeartDisease
import config
...
def get_heart_disease():
    if request.method == "GET":
        ...
        if disease == 1:
            return render_template("index.html",prediction="The patient has a heart disorder, he should go to the hospital and get well soon.")
        else:
            return render_template("index.html",prediction='The patient has not heart disease, he should live his next life happily.')

        # return render_template("index.html", prediction = disease)
    # if disease == 1:
    #     return jsonify({'Result':f"{disease} patient has a heart disease"})
    # else:
    #     return jsonify({'Result':f"patient has {disease} heart disease"})
    # if disease == 1:
    #     return jsonify({'Result':'yes patient has a heart disease'})
    # else:
    #     return jsonify({'Result':'patient has no heart disease'})
    else:
